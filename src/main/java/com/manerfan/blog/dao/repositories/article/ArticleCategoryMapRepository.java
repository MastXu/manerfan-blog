...
public interface ArticleCategoryMapRepository
        extends BasicJpaRepository<ArticleCategoryMap, ArticleEntity> {
     ...
     * @return  该文章的分类
     */
    @Query("select map.category from ArticleCategoryMap map where map.article.uid = ?1")
    public List<CategoryEntity> findByArticleUid(long uid);
     ...
     */
    @Modifying
    @Query("delete from ArticleCategoryMap map where map.article.uid = ?1")
    void deleteByArticleUid(long uid);
     ...
     */
    @Modifying
    @Query("delete from ArticleCategoryMap map where map.category.name = ?1")
