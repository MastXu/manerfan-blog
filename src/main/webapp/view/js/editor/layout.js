define([
            ...
            if (this.isOpen) {
                ...
                if (backdrop) { // 显示遮罩
                    $backdropElt = $(utils.createBackdrop(wrapperL1.elt)).on('click.backdrop', _.bind(function () {
                        this.toggle(false); // 点击遮罩的时候隐藏
                    }, this));
                    this.$elt.addClass('bring-to-front');
                }
                transitionEndCallbacks.push(_.bind(function () { // 当css过渡效果完成时的回调
                    if (--pushedEvents === 0) {
                        if (this.isOpen) {
    ...
    layout.init = function () {
        ...
        wrapperL1 = new DomObject('.layout-wrapper-l1'); // 第一层
        ...
        documentPanel = new DomObject('.document-panel'); // 右侧侧拉面板

        editor = new DomObject('#wmd-input'); // 编辑区
        editorContentElt = editor.elt.querySelector('.editor-content'); // 编辑区内容
        editorMarginElt = editor.elt.querySelector('.editor-margin'); // 编辑区左侧边栏 包含注释

        previewPanel = new DomObject('.preview-panel'); // 预览区
        previewContainer = new DomObject('.preview-container'); // 预览容器
        previewContentElt = document.getElementById('preview-contents'); // 预览区内容
        navbarToggler = new DomObject('.layout-toggler-navbar'); // 隐藏工具条按钮
        previewToggler = new DomObject('.layout-toggler-preview'); // 隐藏预览区按钮
        previewResizer = new DomObject('.layout-resizer-preview'); // 调整预览区宽度按钮
        previewButtons = new DomObject('.extension-preview-buttons'); // 帮助、统计等浮动工具条

        navbarDropdownElt = navbar.elt.querySelector('.buttons-dropdown .dropdown-menu'); // 分辨率过低时，会显示一个下拉菜单
        $navbarDropdownBtnElt = navbar.$elt.find('.buttons-dropdown');

        navbarTitleContainerElt = navbar.elt.querySelector('.title-container'); // 文章标题
        $navbarTitleElt = navbar.$elt.find('.file-title-navbar, .input-file-title'); // 文章标题内容
